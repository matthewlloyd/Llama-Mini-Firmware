...
void eeprom_llama_init(void) {
    ...
    if (!eeprom_llama_check_crc32()) {
        ...
        if (_eeprom_llama_cache.VERSION == 1) {
            ...
            memcpy((void*)&_eeprom_llama_cache.EXTRUDER_TYPE, (void*)&_eeprom_llama_cache.DATASIZE, 19);
            eeprom_llama_set_var(EEVAR_LLAMA_EXTRUDER_REVERSE, variant8_ui8(0), false);
            if (_eeprom_llama_cache.EXTRUDER_TYPE == 2) {  // Bond-Rev
                eeprom_llama_set_var(EEVAR_LLAMA_EXTRUDER_TYPE, variant8_ui8(1), false);
                eeprom_llama_set_var(EEVAR_LLAMA_EXTRUDER_REVERSE, variant8_ui8(1), false);
            } else if (_eeprom_llama_cache.EXTRUDER_TYPE == 3) {  // Custom
                eeprom_llama_set_var(EEVAR_LLAMA_EXTRUDER_TYPE, variant8_ui8(2), false);
            }
            eeprom_llama_set_var(EEVAR_LLAMA_VERSION, variant8_ui16(2), false);
            eeprom_llama_set_var(EEVAR_LLAMA_DATASIZE, variant8_ui16(EEPROM_LLAMA_DATASIZE), true);
        }
    }
